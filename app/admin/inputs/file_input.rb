class FileInput < Formtastic::Inputs::FileInput
  ...
  def image_preview_html
    template.image_tag(@object.send(method).url, :class => "image-preview image-preview-custom", :style => "width:180px")
    # template.image_tag(@object.send(method).url, :class => "image-preview image-preview-custom")
  end
end
